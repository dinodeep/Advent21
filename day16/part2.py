...
def performOperation(typeid, packetValues):
    if typeid == 0:
        # sum of values
        ...
        # no such type id
        print(f"ERROR - IMPROPER TYPEID: {typeid}")
        return -1
...
def parsepacket(binary):
    ...
    if typeid == 4:
        ...
        # compute the whole packet value using the subpacket values and operation type id
        packetvalue = performOperation(typeid, subpacketValues)
...
def main():
    ...
    packetvalue, _, _ = parsepacket(binary)
